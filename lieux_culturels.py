...
#"""**Compléter les coordonnées géographiques manquantes**"""
...
#""**Mettre les colonnes date au bon format**"""
...
#st.subheader("DataFrame 'up_horaires_df' after column removal")
#st.dataframe(up_horaires_df.head())
...
def create_filtered_map(category='Toutes', day='Tous'):
    ...
    st.title(f"Carte filtrée par catégorie : {category}")
    folium_static(carte)
...
categorie_selectionnee = st.sidebar.selectbox('Sélectionner une catégorie:', ['Toutes'] + up_horaires_df['Catégorie'].unique().tolist())
jour_selectionne = st.sidebar.selectbox('Sélectionner un jour de la semaine:', ['Tous'] + up_horaires_df['Jour de la semaine'].unique().tolist())
...
create_filtered_map(category=categorie_selectionnee)
...
# Carte interactive
st.title(f'Carte interactive des lieux nantais en {annee_selectionnee}')
